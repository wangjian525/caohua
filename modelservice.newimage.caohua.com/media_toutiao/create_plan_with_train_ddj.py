# -*- coding:utf-8 -*-
"""
   File Name：     create_plan_with_train_ddj.py
   Description :   新建计划：计划属性自动化组合（大东家）
   Author :        royce.mao
...
from media_toutiao.script.realtime_df_ddj import DataRealTime  ## 采集实时数据类
...
class NewCreatePlan(object):
    ...
    def find_new_images(game_ids):
        ...
        # TODO:3步走过滤要测的新素材（限定那3个包+头条媒体+机器人计划量）
        # TODO:交集的game_ids
    ...
    def plan_dtc_screen(plan, plan_prob, ad_accounts, plan_num):
        ...
        def im_1_ad(plan):
            ...
            for _, row in plan.sort_values('prob', ascending=False).iterrows():
                ...
                elif row['image_id'] not in images and row['ad_account_id'] != ad_accounts_np[ad_counts.argsort()[-1]]:
                    plan_result = plan_result.append(row.to_frame().T)
                    images.append(row['image_id'])
        ...
        # 年龄限制
        plan_result['age'] = plan_result['age'].apply(lambda x: "['AGE_BETWEEN_18_23', 'AGE_BETWEEN_24_30', 'AGE_BETWEEN_31_40', 'AGE_BETWEEN_41_49']")
        ...
        plan_result['cpa_bid'] = plan_result[['cpa_bid','deep_bid_type']].apply(lambda x: 588.88 if x.deep_bid_type == 'BID_PER_ACTION' else x.cpa_bid, axis=1)
        plan_result['operation'] = 'disable'
        plan_result['web_url'] = plan_result['game_id'].apply(lambda x:'https://www.chengzijianzhan.com/tetris/page/7090839861226012685/'
                                                            if x == 1001917 else
        ...
        plan_result['plan_auto_task_id'] = ""
        plan_result['op_id'] = 13678
        plan_result['district'] = 'CITY'
        plan_result['flag'] = plan_result['smart_bid_type'].apply(lambda x:'CX_TT_NB' if x == 'SMART_BID_NO_BID' else 'CX_TT')
    ...
    def __call__(self, ):
        """ 创建类函数 """
        # 提取素材
        new_images = self.find_new_images([1001888,1001890,1001917])  ## 该方法得到新素材
        print("[INFO]：新素材提取完毕！")

        # 新建计划
        plan_create = self.plan_attr_assoc(new_images, [11815,11816,11817])
        plan_result, plan_prob = self.fea_assoc(plan_create)  # 全量组合 + 计划训练
        print("[INFO]: 源计划组合完毕！")

        # 过滤计划
        plan_result = self.plan_dtc_screen(plan_result, plan_prob, [11815,11816,11817], 20)  # 20代表本批次计划总量
 
        # 提交计划
        if len(plan_result[plan_result['game_id'] == 1001888]) > len(plan_result[plan_result['game_id'] == 1001917]) and len(plan_result[plan_result['game_id'] == 1001890]) >= len(plan_result[plan_result['game_id'] == 1001917]):
            print("可以提交！")
...
class FeaAssoc(object):
    """ 计划特征组合类 """
    def __init__(self, ):
        self.game_id1 = 1001917  # 江南3
        self.game_id2 = 1001888  # 江南1
        self.game_id3 = 1001890  # 玩经商

        self.ad_account_id1 = [11815]
        self.ad_account_id2 = [11816]
        self.ad_account_id3 = [11817]
    ...
    def __call__(self, plan):
        """ 组合类函数 """
        # 默认值
        plan['game_id'] = plan['ad_account_id'].apply(lambda x:self.game_id1 if x in self.ad_account_id1 else (self.game_id2 if x in self.ad_account_id2 else self.game_id3))
        plan['platform'] = 1
        ...
        # 选budget
        plan['budget'] = plan['platform'].apply(lambda x: 2500 if x == '[ANDROID]' else 3000)

        # 选'ad_keywords', 'title_list', 'third_industry_id'  创意
        plan_jn = plan[plan['game_id'].isin([self.game_id1, self.game_id2])]
        df_create_jn = self.df_create[self.df_create['game_id'].isin([self.game_id1, self.game_id2])]
        plan_js = plan[plan['game_id'] == self.game_id3]
        df_create_js = self.df_create[self.df_create['game_id'].isin([self.game_id3])]

        sample_df_jn = df_create_jn[['ad_keywords', 'title_list', 'third_industry_id']]
        sample_df_jn = sample_df_jn.sample(n=plan_jn.shape[0], replace=True).reset_index(drop=True)

        sample_df_js = df_create_js[['ad_keywords', 'title_list', 'third_industry_id']]
        sample_df_js = sample_df_js.sample(n=plan_js.shape[0], replace=True).reset_index(drop=True)

        sample_df = sample_df_jn.append(sample_df_js).reset_index(drop=True)
        ...
        plan['roi_goal'] = plan['deep_bid_type'].apply(lambda x:0.015 if x=='ROI_COEFFICIENT' else np.nan)
