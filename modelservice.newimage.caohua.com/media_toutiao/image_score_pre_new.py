...
class NewImageScore(object):
    ...
    def get_new_images(mode):
        ...
        if mode == '末日':
            sql = 'select image_id,image_name,label_ids,game_ids,score,label from test.new_image_test'  ## 未测过新的素材查询
        if mode == '帝国':
            sql = 'select image_id,image_name,label_ids,game_ids,score,label from test.new_image_test_dg'  ## 未测过新的素材查询
