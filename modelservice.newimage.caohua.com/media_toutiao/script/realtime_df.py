...
class DataRealTime(object):
    ...
    def get_now_plan_roi():
        ...
        sql = '''
            ...
            WHERE
                b.tdate >= date( NOW() - INTERVAL 720 HOUR )  # 30天内8个点的好计划
                AND b.tdate_type = 'day'
                AND b.media_id = 10
                AND (b.game_id = 1001703 OR b.game_id = 1001772) 
                AND b.amount >= 1000
                AND b.pay_role_user_num >= 1
                AND b.new_role_money >= 100
                AND (b.new_role_money / b.amount)>=0.1
                '''
        cur.execute(sql)
    ...
    def __call__(self, ):
        ...
        df['label'] = df.apply(lambda x: 1 if x.new_role_money / x.amount >= 0.015 else (0 if x.new_role_money / x.amount < 0.005 else -1), axis=1)
        ...
        print("候选计划deep_bid_type分布：", df_create['deep_bid_type'].value_counts())
        print("===================")
        print("候选计划smart_bid_type分布：", df_create['smart_bid_type'].value_counts())
        print("===================")
        print("候选计划inventory_type分布：", df_create['inventory_type'].value_counts())
        print("===================")
        print("候选计划title_list分布：", df_create['title_list'].value_counts())
