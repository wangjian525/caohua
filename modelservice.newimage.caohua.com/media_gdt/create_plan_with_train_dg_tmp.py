...
"""
   ...
   Description :   新建计划：测新计划自动化组合（帝国广点通）（不入库）
   Author :        royce.mao
...
from media_gdt.script.realtime_df_dg import DataRealTime, get_game_id, image_elements, location_type  ## 采集实时数据类
...
class NewCreatePlan(object):
    ...
    def plan_dtc_screen(plan_prob, plan, plan_out_num, is_test=True):
        ...
        plan_result['game_name'] = '罗马争端' ## TODO:1001545
        plan_result['platform'] = 1 ## TODO:新字段
        plan_result['budget_mode'] = 0  # 不限预算
        plan_result['daily_budget'] = 0  # 不限预算
        plan_result['promoted_object_id'] = '1111430997' ## TODO:1545对应的应用ID
        ...
        profile_id_dict = {'8849': '535651', '8850': '535663', '8851': '535667', '8852': '535671', '8853': '535673'}  ## TODO:测新账号
...
class FeaAssoc(object):
    ...
    def __init__(self, ):
        ...
        self.df_create = self.df_create[self.df_create['game_id'].isin([1001545, 1001465])]
        print("[INFO]: 实时数据提取完毕！")  ## 测试用日志

    @staticmethod
    def goal_given(data, plan_sets, col_sets):
        """[函数：版位下优化目标依赖]
        ...
        def req_param(row):
            ...
            if row['site_set'] == "['SITE_SET_MOMENTS']" or row['site_set'] == "['SITE_SET_WECHAT']":
                ## 微信版位
                elements, key = {'video':0}, 'video'  ## 初始化
                while len(str(elements[key])) != 6:
                    elements = plan_sets[(plan_sets['site_set'] == row['site_set']) & (plan_sets['adcreative_template_id'] == row['adcreative_template_id'])]['adcreative_elements'].sample(1).item()
                    key = list(set(['short_video_struct','video']).intersection(set(list(elements.keys()))))[0]
                    if key == 'short_video_struct':
                        break
                row['adcreative_elements'] = elements
                row[col_sets] = plan_sets[(plan_sets['site_set'] == row['site_set']) & (plan_sets['adcreative_template_id'] == row['adcreative_template_id'])][col_sets].sample(1).values.squeeze()
            else: 
                ## 非微信版位
                ...
                row['adcreative_elements'] = elements
                row[col_sets] = plan_sets[(plan_sets['adcreative_template_id'] == row['adcreative_template_id']) & (plan_sets['site_set'] != "['SITE_SET_MOMENTS']") & (plan_sets['site_set'] != "['SITE_SET_WECHAT']")][col_sets].sample(1).values.squeeze()
    ...
    def __call__(self, plan):
        ...
        ## TODO:附加：优化目标依赖
        plan = plan.groupby(['image_id']).apply(lambda x:self.goal_given(x, self.df_create.copy(), self.gdt_orient_cols_bid.copy()))
