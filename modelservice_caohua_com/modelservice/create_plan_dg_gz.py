...
class CreatePlanDggz:
    ...
    def Process(self):
        main_model()
        ret = json.dumps({"code": 200, "msg": "success!", "data": "dg gz create plan is  success"})
        return ret
...
def get_all_data():
    # 读取历史数据
    plan_info = pd.read_csv('./hist_data_dg/ptom_third_plan.csv')
    #     image_info = pd.read_csv('./hist_data_dg/image_info.csv')
    #     launch_report = pd.read_csv('./hist_data_dg/launch_report.csv')
...
def get_score_image():
    ...
    sql = 'select image_id,label_ids from dws.dws_image_score_d where media_id=10 and score>=520 and dt=CURRENT_DATE group by image_id,label_ids'
...
def create_plan(df, score_image):
    ...
    ad_account_id_group = np.array([7711, 7708, 7709, 7710])     # 广州帝国
    game_id = 1001619
    df = df[df['game_id'].isin([1001619, 1001685, 1001657])]
    # df = df[df['game_id'] == game_id]  # 素材不能冲突
    ...
    # ad_account_id_group = np.array([7711, 7708, 7709, 7710])
    # image_id_group = np.intersect1d(df['image_id'].unique(), score_image)
    # # print(image_id_group)
    # df = df[df['deep_bid_type'].isin(['BID_PER_ACTION', 'ROI_COEFFICIENT'])]
    # df = df[df['image_id'].isin(image_id_group)]
...
def main_model():
    ...
    # plan_result['cpa_bid'] = plan_result['cpa_bid'].apply(lambda x: x if x <= 550 else 550)
    plan_result['cpa_bid'] = plan_result['cpa_bid'].apply(lambda x: x if x >= 600 else random.randint(600, 650))
    ...
    plan_result['web_url'] = 'https://www.chengzijianzhan.com/tetris/page/6977192075767758862/'
