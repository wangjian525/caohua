...
class CreatePlangdtjp:
    ...
    def Process(self):
        main_model()
        ret = json.dumps({"code": 200, "msg": "success!", "data": "create jp gdt plan is success"})
        return ret
...
def get_game_id():
    ...
    sql = '''
        SELECT dev_game_id AS game_id FROM db_data.t_game_config WHERE game_id = 1051 AND dev_game_id IS NOT NULL  ## TODO
    '''
    cur.execute(sql)
...
def get_image_info():
    ...
    sql = '''
        ...
        WHERE
            a.game_id IN ( SELECT dev_game_id AS game_id FROM db_data.t_game_config WHERE game_id = 1051 AND dev_game_id IS NOT NULL )  ## TODO 
            AND a.media_id = 16 
...
def get_plan_json(plan_info):
    ...
    # plan_info = plan_info[plan_info['auto_audience'] == False]
    plan_info = plan_info[['ad_account_id', 'game_id', 'channel_id', 'source_id', 'budget_mode',
                           ...
                           'behavior_category_id_list','site_set',
...
def get_score_image():
    ...
    sql = 'select image_id,label_ids from dws.dws_image_score_d where media_id=16 and score>=500 and dt>=date_sub(CURRENT_DATE,3) group by image_id,label_ids'
...
def get_now_plan_roi():
    ...
    sql = '''
        ...
        WHERE
            b.tdate >= date( NOW() - INTERVAL 360 HOUR ) 
            AND b.tdate_type = 'day' 
            AND b.media_id = 16 
            AND b.platform = 2
            AND b.game_id IN ({}) 
            ...
            AND (b.new_role_money / b.amount)>=0.0025
    '''
    finalSql = sql.format(game_id)
    cur.execute(finalSql)
    result_df = pd.read_sql(finalSql, conn)
...
def create_plan(df, score_image):
    ...
    # df = df[df['game_id'].isin([1001379, 1001703])]
    ad_account_id_group = np.array([8849,8850,8851,8852,8853])
    ## TODO:高分素材少
    ...
    print("500评分素材量：", len(score_image), "最后交集素材量：", len(image_id_group))
    plan = pd.DataFrame()
...
def get_train_df():
    ...
    creative_info = creative_info[['channel_id', 'source_id', 'image_id', 'deep_link_url',
                                   'adcreative_template_id', 'page_spec', 'page_type', 'label',
                                   'promoted_object_id', 'promoted_object_type', 'automatic_site_enabled',
                                   'link_name_type', 'adcreative_elements']]  ## 'link_page_type', 'profile_id', 'link_page_spec'
    ...
    plan_info['profile_id'] = np.nan
    plan_info['link_page_type'] = np.nan
    plan_info['link_page_spec'] = np.nan
    ...
    df_create = pd.merge(plan_info_current, image_info, on=['channel_id', 'source_id', 'image_id'], how='inner')
    print("最近计划数量（过滤纯买激活）：", len(plan_info_current), "第一次拼接数量：", len(df_create))
    df_create = pd.merge(df_create, now_plan_roi.drop(['ad_account_id'], axis=1), on=['channel_id', 'source_id'], how='inner')

    print("roi好计划数量：", len(now_plan_roi), "第二次拼接数量：", len(df_create))  ## TODO:plan_info不能删除纯买激活计划，不然计划太少了
...
def get_ad_create(plan_result):
    ...
    open_api_url_prefix = "http://192.168.0.60:8085/"  ## "https://ptom.caohua.com/"
...
def main_model():
    ...
    plan_result['site_set_copy'] = plan_result['site_set'].copy()
    plan_result['site_set'] = plan_result['site_set'].map(str)

    # # TODO:随机投几个朋友圈版位
    # if len(plan_result[plan_result['site_set'] == "['SITE_SET_MOMENTS']"]) == 0:
    #     plan_result = plan_result['site_set']
    ...
    # 修改落地页ID  # TODO:小程序、朋友圈的落地页ID关系
    plan_result['page_spec'] = plan_result.apply(
        lambda x: {'override_canvas_head_option': 'OPTION_CREATIVE_OVERRIDE_CANVAS',
                   'page_id': '2250094336'} if x.site_set == "['SITE_SET_MOMENTS']"
        else ({'override_canvas_head_option': 'OPTION_CREATIVE_OVERRIDE_CANVAS',
               'page_id': '2249955092'} if x.site_set == "['SITE_SET_WECHAT']" else np.nan), axis=1)
    plan_result['link_page_spec'] = plan_result.apply(
        lambda x: {'page_id': '2250094336'} if x.site_set == "['SITE_SET_MOMENTS']"
        else ({'page_id': '2249955092'} if x.site_set == "['SITE_SET_WECHAT']" else np.nan), axis=1)

    # 修改朋友圈头像ID  # TODO:朋友圈头像ID与账号对应关系
    ...
    profile_id_dict = {'8849': '511264', '8850': '511297', '8851': '513556', '8852': '513574', '8853': '513584'}
    ...
    plan_result['flag'] = 'GDT_JP_IOS'
    plan_result['promoted_object_id'] = '1548099395'
    ...
    plan_result['site_set'] = plan_result['site_set_copy']

    plan_result['platform'] = 2
    plan_result['game_name'] = "女神驾到"  ## TODO：新增字段
    # 周三周四更新，凌晨不跑计划
    ...
    plan_result.drop(['label','page_type','site_set_copy'], axis=1, inplace=True)
    plan_result.to_csv('./plan_result_jp.csv', index=0)  # 保存创建日志
