...
def get_all_data():
    # 读取历史数据
    plan_info = pd.read_csv('./hist_data_mr/ptom_third_plan.csv')
    image_info = pd.read_csv('./hist_data_mr/image_info.csv')
    launch_report = pd.read_csv('./hist_data_mr/launch_report.csv')
...
def create_plan(df, score_image):
    ...
    df = df[df['game_id'].isin([1001379, 1001703, 1001756])]
    ad_account_id_group = np.array([6989, 6866, 7774, 7775, 6990])
...
def main_model():
    ...
    #     plan_result = plan_result.sample(20)
    ...
    # plan_result['cpa_bid'] = plan_result['cpa_bid'].apply(lambda x: x if x >= 630 else random.randint(600, 650))
    plan_result['cpa_bid'] = plan_result['cpa_bid'].apply(lambda x: random.randint(600, 650))
    # plan_result['operation'] = 'disable'
    plan_result['web_url'] = 'https://www.chengzijianzhan.com/tetris/page/6991718208874512391/'
