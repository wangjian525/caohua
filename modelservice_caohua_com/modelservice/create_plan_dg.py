...
def create_plan(df, score_image):
    ...
    df = df[df['game_id'].isin([1001788, 1001784, 1001668, 1001792])]
    ad_account_id_group = np.array([9046, 9047, 9048, 9049, 9050])
    # ad_account_id_group = np.array([9046])
...
def doSubStr(x):
    if '罗马单机版' in x:
        res = re.sub('罗马单机版', '亚洲王朝', x)
    elif '罗马单机加强版' in x:
        res = re.sub('罗马单机加强版', '亚洲王朝', x)
    elif '领主荣耀' in x:
        res = re.sub('领主荣耀', '亚洲王朝', x)
    elif '帝国' in x:
        res = re.sub('帝国', '帝王', x)
    else:
        res = x
...
def main_model():
    ...
    if plan_result.shape[0] > 70:
        plan_result = plan_result.sample(70, weights=plan_result['weight'])
    ...
    plan_result['cpa_bid'] = plan_result['cpa_bid'].apply(lambda x: random.randint(600, 640))
    ...
    plan_result['web_url'] = 'https://www.chengzijianzhan.com/tetris/page/7011089102134378527/'
    ...
    plan_result['title_list'] = plan_result['title_list'].apply(doSubStr)
