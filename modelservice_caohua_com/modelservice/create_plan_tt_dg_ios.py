...
warnings.filterwarnings('ignore')
logger = logging.getLogger('CreatePlanttdgIOS')


#
# 打包接口
#
class CreatePlanttdgIOS:
    def POST(self):
        # 处理POST请求
    ...
    def Process(self):
        main_model()
        ret = json.dumps({"code": 200, "msg": "success!", "data": "create tt dg ios plan is success"})
        return ret
...
def get_score_image():
    ...
    sql = 'select image_id,label_ids from dws.dws_image_score_d where media_id=10 and score>=550 and dt>=date_sub(current_date, 60) group by image_id,label_ids'
...
def get_current_image():
    ...
    cursor = conn.cursor()
    sql_engine = 'set hive.execution.engine=tez'
    sql = 'select image_id,label_ids from dws.dws_image_score_d where media_id=10 and score>=500 and dt= current_date group by image_id,label_ids'
    cursor.execute(sql_engine)
    cursor.execute(sql)
    result = as_pandas(cursor)
    result['label_ids'] = result['label_ids'].astype(str)
...
def create_plan(df, score_image):
    # df = df[df['game_id'] == game_id]  1001792
    ...
    ad_account_id_group_1 = np.array([8100, 8101, 8102, 8103, 8152])  ## 称霸包账号
    ad_account_id_group_2 = np.array([8187, 8188, 8189, 8190, 8191])  ## 帝都包账号
    ad_account_id_group_3 = np.array([8153, 8154, 8155, 8156, 8157])  ## 帝国包账号
    ad_account_id_group_4 = np.array([8182, 8183, 8184, 8185, 8186])  ## 文明包账号
    ...
    def plan_cut(df, ad_account_id_group, image_id_group, package="称霸"):
        """ 闭包区分：计划要素 """
        image_id_group = random.sample(list(image_id_group), 15) if len(image_id_group) > 15 else image_id_group
        ...
        if package=="称霸":
            ...
            df = df[df['game_id']==1001701]
        elif package=="帝都":
            plan['game_id'] = 1001783
            df = df[df['game_id']==1001783]
        elif package=="帝国":
            plan['game_id'] = 1001608
            df = df[df['image_id'].isin(image_id_group)]  ## df = df[df['game_id']==1001608]
        elif package=="文明":
            plan['game_id'] = 1001657
            df = df[df['game_id']==1001657]

        # 选platform测试期默认[ANDROID]
        # print(package, len(df))
        plan['platform'] = 2
        ...
        # 年龄定向写死
        plan['age'] = 'AGE_BETWEEN_18_23,AGE_BETWEEN_24_30,AGE_BETWEEN_31_40,AGE_BETWEEN_41_49'
...
def get_train_df():
    ...
    df['label'] = df.apply(lambda x: 1 if x.new_role_money / x.amount >= 0.015 else 0, axis=1)
    ...
    df_create = df_create[df_create['platform'] == 2]
...
def doSubStr(row):
    """ 推广文案过滤 """
    dic = {1001701:'称霸时代',1001783:'帝都战纪',1001608:'帝国战魂',1001657:'文明启航'}
    x = row.copy()

    if '罗马单机版' in x['title_list']:
        res = re.sub('罗马单机版', dic[x['game_id']], x['title_list'])
    elif '罗马单机加强版' in x['title_list']:
        res = re.sub('罗马单机加强版', dic[x['game_id']], x['title_list'])
    elif '领主荣耀' in x['title_list']:
        res = re.sub('领主荣耀', dic[x['game_id']], x['title_list'])
    elif '帝国单机版' in x['title_list']:
        res = re.sub('帝国单机版', dic[x['game_id']], x['title_list'])
    else:
        res = x['title_list']
    return res
...
def main_model():
    ...
    if plan_result.shape[0] > 100:  ## TODO：计划数量
    ...
    plan_result = plan_result.groupby(['image_id','game_id']).apply(lambda x: x.sample(1) if len(x) > 1 else x)  ## TODO:素材下的计划量不能太多

    plan_result = plan_result.drop(['create_time', 'create_date', 'prob', 'rank_ad_im', 'label_ids', 'weight'], axis=1)
    plan_result['convertIndex'] = plan_result['deep_bid_type'].apply(lambda x:6 if x == 'BID_PER_ACTION' else 7)
    ...
    # plan_result['cpa_bid'] = plan_result['cpa_bid'].apply(lambda x: random.randint(800, 900) if x > 900 else x)  ## TODO:(待修改)
    ...
    plan_result['city'] = plan_result['city'].apply(
        lambda x: x if x != [] else [11, 12, 13, 14, 15, 21, 22, 23, 31, 32, 33, 34, 35, 36, 37, 41, 420200, 420300,
                                     ...
                                     65, 71, 81, 82])
    # plan_result['action_days'] = plan_result['action_days'].convert_dtypes()
    # plan_result['operation'] = 'disable'
    plan_result['op_id'] = 13268 # 13678
    plan_result['district'] = 'CITY'
    plan_result['flag'] = 'SSR_IOS'
    ...
    # plan_result = plan_result.iloc[:3]
