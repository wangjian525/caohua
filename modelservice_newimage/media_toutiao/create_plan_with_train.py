...
from media_toutiao.script.realtime_df import DataRealTime, get_game_id  ## 采集实时数据类
from media_toutiao.image_score_pre_new import NewImageScore  ## 新素材事前评分类
...
class NewCreatePlan(object):
    ...
    def find_new_images(label_id, true_game_ids, is_back=True):
        ...
        # 新素材的label_ids对齐
        image_df = image_df.loc[image_df.apply(lambda x:True if not pd.isnull(x['label_ids']) and str(label_id) in x['label_ids'] else False, axis=1)]
        # 新素材的game_ids对齐
        ...
        if not is_back:
            ...
            image_df_temp = image_df.loc[image_df.apply(lambda x:True if not any(plan_df['image_id'] == x.image_id) else False, axis=1)]
            print("pure new num:", len(image_df_temp))  ## 纯新素材数量
            
            plan_df = plan_df.groupby('image_id')['amount'].sum()
            plan_df = pd.DataFrame({'image_id':plan_df.index,'amount':plan_df.values})
            plan_df = plan_df.loc[(plan_df['amount'] >= 1000) & (plan_df['amount'] <= 6000)]  # todo:上下限调整
        ...
        # os.system("hadoop fs -put {} hdfs://masters/user/hive/warehouse/test.db/new_image_test".format(csv_path))
    ...
    def plan_dtc_screen(plan_prob, plan, plan_out_num, is_test=True):
        ...
        plan_result['cpa_bid'] = plan_result['cpa_bid'].apply(lambda x: x if x >= 600 and x <=650 else random.randint(600, 650))
    ...
    def __call__(self, ):
        ...
        # 计划：创建 + 建模
        plan_associate = self.plan_attr_assoc(score_images_associate, cfg.AD_ACCOUNT_ID_GROUP_MR, cfg.ENCODER_COLS + cfg.HOTTING_COLS + cfg.CONT_COLS, cfg.AD_ACCOUNT_CAPACITY)
