...
class DataRealTime(object):
    ...
    def get_score_images():
        ...
        sql = 'select image_id,label_ids,model_run_datetime,score from dws.dws_image_score_d where media_id=10 and score>=550 and dt=CURRENT_DATE'
    ...
    def __call__(self, cfg):
        ...
        image_info_new = self.get_image_info()
        launch_report_new = self.get_launch_report(game_id_dict)
        ...
        df = df.loc[df.apply(lambda x:True if not None in x['inventory_type'] else False, axis=1)]
        ...
        df_create = df_create[df_create['deep_bid_type'].isin(['BID_PER_ACTION'])]  ## 'ROI_COEFFICIENT'
