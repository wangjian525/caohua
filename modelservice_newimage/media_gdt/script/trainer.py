...
def train(df, image, plan, cfg):
    ...
    plan['y'] = 0  ## 初始化
    cols = cfg.GDT_ENCODER_COLS + cfg.GDT_HOTTING_COLS + cfg.GDT_CONT_COLS + ['create_time', 'y'] ## 初始化
    df_mesh = plan[cols].append(df[cols])  ## 合并做特征工程

    df_mesh = get_mutil_feature(df_mesh, cfg.GDT_HOTTING_COLS)
    for col in list(set(cfg.GDT_ENCODER_COLS) - set(['image_id'])):
        le = LabelEncoder()
        df_mesh[col] = df_mesh[col].astype(str)
        df_mesh[col] = le.fit_transform(df_mesh[col])
        
    df = df_mesh.iloc[len(plan):]  ## 切分
    plan_after = df_mesh.iloc[:len(plan)].drop(['image_id', 'create_time', 'y'], axis=1)  ## 切分
    ...
    df['image_score'] = df.apply(lambda x: _score(x, image), axis=1)
    ...
    joblib.dump(xgb_model, cfg.GDT_DTC_PLAN_PATH)
