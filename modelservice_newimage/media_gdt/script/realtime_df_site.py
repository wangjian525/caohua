...
class DataRealTime(object):
    ...
    def get_now_plan_roi(game_id):
        ...
        sql = '''
            ...
            WHERE
                b.tdate >= date( NOW() - INTERVAL 1080 HOUR )  ## 第1次跑版位，由于公众号最近跑的少，还是跑付费次数721不跑首次付费560
                AND b.tdate_type = 'day'
                AND b.media_id = 16
                AND b.game_id IN ({})
                AND b.amount >= 500
        ...
        '''
        finalSql = sql.format(game_id)    ## TODO:修改
        cur.execute(finalSql)
        result_df = pd.read_sql(finalSql, conn)
        cur.close()
        conn.close()
    ...
    def __call__(self, cfg):
        ...
        # print(df_create[df_create['site_set'] == "['SITE_SET_WECHAT']"][['image_id','adcreative_template_id','adcreative_elements']])
