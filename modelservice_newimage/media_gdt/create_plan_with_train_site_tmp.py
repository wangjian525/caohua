...
"""
   ...
   Description :   新建计划：测新计划固定版位测试（末日广点通）（拆版位且不入库）
   Author :        royce.mao
   date：          2021/7/5 20:25
"""
...
class NewCreatePlan(object):
    ...
    def plan_attr_assoc(score_image_group, ad_account_id_group, capacity):  # 具体建模过程见Plan.ipynb
        ...
        # 复制3条
        plan = pd.DataFrame(np.repeat(plan.values, 3, axis=0), columns=['image_id','label_ids','image_score'])  ## TODO:复制3条
        ...
        for image_id in dist_image_ids:
            ad_asso = ad_account_id_group[ad_counts.argsort()[:3]]  ## TODO:使用次数最少的4个账号
            plan['ad_account_id'].loc[plan['image_id'] == image_id] = ad_asso  ## ad_account_id_group.tolist()
            # ad_counts += 1
            ad_counts[ad_counts.argsort()[:3]] += 1  ## TODO:使用次数最少的4个账号
        # 容积过滤
    ...
    def plan_dtc_screen(plan_prob, plan, plan_out_num, is_test=True):
        ...
        ## 子素材以版位尺寸匹配结果做新素材的替换
        plan_result['adcreative_elements'] = plan_result.apply(lambda x:image_elements(x), axis=1)  ## TODO:
    ...
    def __call__(self, ):
        ...
        # 计划：创建 + 建模
        plan_associate = self.plan_attr_assoc(score_images_associate, cfg.GDT_AD_ACCOUNT_ID_GROUP_MR, cfg.AD_ACCOUNT_CAPACITY)
...
class FeaAssoc(object):
    ...
    def site_given(df, plans):
        ...
        # TODO:版位改变（朋友圈+公众号+优量汇+2种随机普通版位的组合！）
        def req_site(data, plan_sets):
            """ 计划版位分配的闭包处理 """
            score = data['image_score'].iloc[0]
            
            moments = data[data['site_set'] == "['SITE_SET_MOMENTS']"].sample(n=1).reset_index(drop=True) if len(data[data['site_set'] == "['SITE_SET_MOMENTS']"]) > 1 else \
            ...
            # wechats = data[data['site_set'] == "['SITE_SET_WECHAT']"].sample(n=1).reset_index(drop=True) if len(data[data['site_set'] == "['SITE_SET_WECHAT']"]) > 0 else \
            #           plan_sets[plan_sets['site_set'] == "['SITE_SET_WECHAT']"].sample(n=1).reset_index(drop=True)
            # wechats['image_score'], wechats['platform'] = score, 1
            # wechats = wechats[data.columns]

            # union =   data[data['site_set'] == "['SITE_SET_MOBILE_UNION']"].sample(n=1).reset_index(drop=True) if len(data[data['site_set'] == "['SITE_SET_MOBILE_UNION']"]) > 0 else \
            #           plan_sets[plan_sets['site_set'] == "['SITE_SET_MOBILE_UNION']"].sample(n=1).reset_index(drop=True)
            # union['image_score'], union['platform'] = score, 1
            # union = union[data.columns]

            grades =  plan_sets[plan_sets['site_set'].apply(lambda x:True if "," in x else False)].sample(n=2).reset_index(drop=True)
            grades['image_score'], grades['platform'] = score, 1
            grades = grades[data.columns]
            
            plan = pd.concat([moments, grades], axis=0).reset_index(drop=True)  ## TODO:3条计划堆叠拼接
