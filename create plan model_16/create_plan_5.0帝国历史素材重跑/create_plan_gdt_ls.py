...
def get_score_image():
    ...
    sql = 'select image_id,label_ids from dws.dws_image_score_d where media_id=16 and score>=550 and dt>= date_sub(current_date,90)group by image_id,label_ids'
...
def create_plan(df, score_image):
    ...
    # 选出价
    sample_df = df[['optimization_goal', 'bid_strategy', 'bid_amount', 'deep_conversion_type', 'deep_conversion_behavior_spec',
         'deep_conversion_worth_spec']]
    # 包体初期限制为每次付费出价（不能跑ROI出价）
    sample_df = sample_df[sample_df['deep_conversion_worth_spec'].isna()]
    sample_df = sample_df.sample(n=plan.shape[0], replace=True).reset_index(drop=True)
...
def main_model():
    ...
    for ad in np.setdiff1d(plan_create['image_id'].values, ad_num[ad_num > 2].index):
        add_plan = plan_result_pr[plan_result_pr['image_id'] == ad].sort_values('prob', ascending=False)[0:4]
        plan_result = plan_result.append(add_plan)
    ...
    # 只跑朋友圈和
    plan_result = plan_result[(plan_result['site_set'] == "['SITE_SET_MOMENTS']") | (plan_result[ 'site_set'] == "['SITE_SET_QQ_MUSIC_GAME', 'SITE_SET_KANDIAN', 'SITE_SET_TENCENT_NEWS', 'SITE_SET_TENCENT_VIDEO']")]

    plan_num = plan_create['image_id'].nunique() * 6
    ...
    # 除朋友圈外的版外，跑首次付费
    plan_result['optimization_goal'] = plan_result.apply(
        lambda x: 'OPTIMIZATIONGOAL_FIRST_PURCHASE' if x.site_set != "['SITE_SET_MOMENTS']" else x.optimization_goal,
        axis=1)
    plan_result['bid_amount'] = plan_result.apply(
        lambda x: random.randint(220000, 240000) if x.site_set != "['SITE_SET_MOMENTS']" else x.bid_amount, axis=1)
    plan_result['deep_conversion_type'] = plan_result.apply(
        lambda x: np.nan if x.site_set != "['SITE_SET_MOMENTS']" else x.deep_conversion_type, axis=1)
    plan_result['deep_conversion_behavior_spec'] = plan_result.apply(
        lambda x: np.nan if x.site_set != "['SITE_SET_MOMENTS']" else x.deep_conversion_behavior_spec, axis=1)
    plan_result['deep_conversion_worth_spec'] = plan_result.apply(
        lambda x: np.nan if x.site_set != "['SITE_SET_MOMENTS']" else x.deep_conversion_worth_spec, axis=1)
