...
def main_model():
    ...
    ad_account_id_group = np.array([8814, 8824, 8825, 8826])
    plan_num = 8
    ...
    plan_result['deep_conversion_worth_spec'] = plan_result['optimization_goal'].apply(lambda x: {'goal': 'GOAL_1DAY_PURCHASE_ROAS','expected_roi': 0.014} if x == 'OPTIMIZATIONGOAL_APP_ACTIVATE' else np.nan)
    plan_result['bid_amount'] = plan_result['optimization_goal'].apply(
        lambda x: random.randint(16000, 17000) if x == 'OPTIMIZATIONGOAL_APP_ACTIVATE'
