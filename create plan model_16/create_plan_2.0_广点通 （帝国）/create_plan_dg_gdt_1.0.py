...
def get_train_df():
    ...
    # df_create = df_create[df_create['site_set'] == "['SITE_SET_MOMENTS']"]
    # df_create['site_set'] = df_create['site_set'].apply(ast.literal_eval)
...
def main_model():
    ...
    # 除朋友圈外的版外，跑首次付费
    plan_result['optimization_goal'] = plan_result.apply(lambda x: 'OPTIMIZATIONGOAL_FIRST_PURCHASE' if x.site_set != "['SITE_SET_MOMENTS']" else x.optimization_goal, axis=1)
    plan_result['bid_amount'] = plan_result.apply(lambda x: random.randint(220000, 240000) if x.site_set != "['SITE_SET_MOMENTS']" else x.bid_amount, axis=1)
    plan_result['deep_conversion_type'] = plan_result.apply(lambda x: np.nan if x.site_set != "['SITE_SET_MOMENTS']" else x.deep_conversion_type, axis=1)
    plan_result['deep_conversion_behavior_spec'] = plan_result.apply(lambda x: np.nan if x.site_set != "['SITE_SET_MOMENTS']" else x.deep_conversion_behavior_spec, axis=1)
    plan_result['deep_conversion_worth_spec'] = plan_result.apply(lambda x: np.nan if x.site_set != "['SITE_SET_MOMENTS']" else x.deep_conversion_worth_spec, axis=1)
