...
"""
   ...
   Description :   数据准备：动态计划的数据提取（广点通帝国）
   Author :        royce.mao
...
def get_score_image():
    ...
    cursor = conn.cursor()
    sql_engine = 'set hive.execution.engine=tez'
    sql = 'select image_id,dt from dws.dws_image_score_d where media_id=16 and score>=600 and dt>=date_sub(current_date, 15)'
    cursor.execute(sql_engine)
    cursor.execute(sql)
    result_1 = as_pandas(cursor)
    sql = 'select image_id,dt from dws.dws_image_score_d where media_id=16 and score>=580 and dt=current_date'
    cursor.execute(sql_engine)
    cursor.execute(sql)
...
def get_good_image():
    ...
    sql = '''
        ...
        WHERE
            ...
            AND a.media_id = 16 
            AND a.create_time >= date( NOW() - INTERVAL 360 HOUR )
        GROUP BY
            a.chl_user_id,
            a.source_id,
            a.image_id
...
def get_R_image():
    ...
    sql = '''
        ...
        WHERE
            ...
            AND b.game_id IN ( SELECT dev_game_id AS game_id FROM db_data.t_game_config WHERE game_id = 1056 AND dev_game_id IS NOT NULL )
            AND b.amount >= 4000
            AND b.new_role_money / b.pay_role_user_num >= 3000
    '''
    cur.execute(sql)
    result = pd.read_sql(sql, conn)
    result['tdate'] = pd.to_datetime(result['tdate'])
...
def get_lz_image():
    ...
    result = result.sort_values('dt')
    result = result.drop_duplicates(['image_id'], keep='last')  ## TODO:保留最近的1次
    result = result[(result['image_run_date_amount']>10000) & (result['image_create_role_pay_sum']>50) & (result['image_create_role_roi']<0.01)]
...
def get_lg_image():
    ...
    result = result.sort_values('dt')
    result = result.drop_duplicates(['image_id'], keep='last')  ## TODO:保留最近的1次
    result = result[result['image_run_date_amount']>100000]
    result = np.intersect1d(result['image_id'], result_df['image_id'])
    # 关闭链接
    cur.close()
    conn.close()
