...
"""
   ...
   Description :   新建计划：动态计划 - 素材&计划配比（广点通末日）
   Author :        royce.mao
...
logger = logging.getLogger('CreatePlanGdtDt')

from utils.get_data_mr_gdt import *

#
# 打包接口
#
class GDTCreatePlanDt:
    def __init__(self,): 
...
class NewCreatePlan(object):
    """ 步骤3：新计划创建类 """
    def __init__(self, ):
        self.ad_account_id_gdt_mr = [7981, 7982, 7984, 8035, 8036]
        self.amount_thresh = 100000  ## TODO:大盘日消耗阈值
        self.amount_thresh_my = 20000  ## TODO:个人盘日消耗阈值
    ...
    def base_market(self,):
        ...
        for day in range(1, 31):
            sum_amount, sum_roi, sum_cost = get_roi_my(0, day, day) 
            sum_roi = sum_roi * 100
            if sum_amount > self.amount_thresh_my*day or day >= 30:
    ...
    def prop_personal(self,):
        ...
        image_score['prop'] = image_score['score_bin'].map({1:base_prop-3, 2:base_prop-2, 3:base_prop-1, 4:base_prop, 5:base_prop+1, 6:base_prop+2})  ## TODO:个性化配比
    ...
    def plan_field(plan):
        ...
        plan['platform'] = 1
        plan['op_id'] = 13678
        plan['flag'] = 'CESHI_GDT_MR'
        plan['game_name'] = '幸存者挑战'
    ...
    def plan_create(self, ):
       ...
       Returns:
        ...
        account_roi = pd.DataFrame({'ad_account_id':self.ad_account_id_gdt_mr, 'roi':[get_roi_peraccount_my(ad) * 100 for ad in self.ad_account_id_gdt_mr]})
        print("账号加权结果：", "\n", account_roi)
...
class FeaAssoc(object):
    ...
    def __call__(self, plan):
        ...
        orient_df = self.df_create[self.gdt_orient_cols_bid].sample(n=plan.shape[0], replace=True, weights=self.df_create['weight']).reset_index(drop=True)  ## TODO:固定权重配比优化
        ...
        orient_df = self.df_create[self.gdt_new_cols].sample(n=plan.shape[0], replace=True, weights=self.df_create['weight']).reset_index(drop=True)  ## TODO:固定权重配比优化
        plan = pd.concat([plan, orient_df], axis=1)
