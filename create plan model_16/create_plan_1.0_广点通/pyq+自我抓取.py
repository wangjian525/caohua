...
def create_plan(df, image_adcreative_elements):
    ...
    sample_df = df[
        ['manager_id', 'expand_enabled', 'expand_targeting', 'device_price', 'app_install_status', 'gender', 'game_consumption_level',
         'age', 'custom_audience', 'excluded_custom_audience', 'network_type', 'conversion_behavior_list', 'excluded_dimension', 'location_types', 'regions',
    ...
    # 计划归因channel_id
    plan['attribute'] = plan.apply(lambda x: [x.manager_id_1], axis=1)
    plan.drop(['manager_id_1'], axis=1, inplace=True)
    plan = plan.reset_index(drop=True)

    return plan


def get_plan(site_set, plan_num):
    ...
    if site_set == 'PYQ':
        ...
        # df_create_s = df_create[(df_create['site_set'] == "['SITE_SET_MOMENTS']")]  ## TODO
    elif site_set == 'XQXS':
        df_create_s = df_create[(df_create['site_set'] == "['SITE_SET_QQ_MUSIC_GAME', 'SITE_SET_KANDIAN', 'SITE_SET_TENCENT_NEWS', 'SITE_SET_TENCENT_VIDEO', 'SITE_SET_MOBILE_UNION']") & (
                                            df_create['adcreative_template_id'] == 720)]  ## TODO
    print('df_create_s', df_create_s.shape)

    ad_account_id_group = np.array([8854, 8843, 8839, 8082, 8817, 8077])
    plan_result = pd.DataFrame()
    for ad_account in ad_account_id_group:
        df_create_a = df_create_s[df_create_s['ad_account_id'] == ad_account]
        print(ad_account, df_create_a.shape[0])
        if df_create_a.shape[0] != 0:
            plan_create_s = create_plan(df_create_a, image_adcreative_elements)
            plan_result_ = plan_create_s.sample(n=plan_num, replace=False).reset_index(drop=True)
            plan_result_['ad_account_id'] = ad_account
        else:
            plan_create_s = create_plan(df_create_s, image_adcreative_elements)
            plan_result_ = plan_create_s.sample(n=plan_num, replace=False).reset_index(drop=True)
            plan_result_['ad_account_id'] = ad_account
        plan_result = plan_result.append(plan_result_)

    # plan_create = create_plan(df_create_s, image_adcreative_elements)
    # ad_account_id_group = np.array([8854, 8843, 8839, 8082, 8817, 8077])    ## TODO
    # # ad_account_id_group = np.array([8035, 8036, 8037, 8038, 8039, 8854, 8843, 8839, 8082, 8817, 8077])
...
def main_model(site_set, plan_num):
    # ad_account_id_group = np.array([8854, 8843, 8839, 8082, 8817, 8077])
    # ad_account_id_group = np.array([8035, 8036, 8037, 8038, 8039, 8854, 8843, 8839, 8082, 8817, 8077])
    plan_result = get_plan(site_set, plan_num)
    ...
    plan_result.drop(["device_price", "app_install_status", "gender", "game_consumption_level", "age", 'custom_audience', 'excluded_custom_audience', "network_type",
                      "excluded_converted_audience", "geo_location",
    ...
    #     plan_result_p['scene_spec'] = plan_result_p['site_set'].apply(
    #         lambda x: {'display_scene': ['DISPLAY_SCENE_NATIVE'],
    ...
    if site_set == 'PYQ':
        ...
        plan_result['adcreative_template_id'] = 720  # 朋友圈竖版素材
...
if __name__ == '__main__':
    main_model(site_set='PYQ', plan_num=6)
    # main_model(site_set='XQXS', plan_num=3)
