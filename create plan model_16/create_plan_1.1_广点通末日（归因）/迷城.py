...
def get_score_imag_7():
    ...
    cursor = conn.cursor()
    sql_engine = 'set hive.execution.engine=tez'
    sql = 'select image_id from tmp_data.tmp_ra_media_platform_image where dt= `current_date`() and platform=1 and media_id=16 and score>=20 and mgame_id=1056 and 7_amount>=1000'
    cursor.execute(sql_engine)
    cursor.execute(sql)
    result = as_pandas(cursor)
...
#     # 选择相关指标达标的计划
#     result = source_df[(source_df['pay_num'] >= 1) & (source_df['pay_cost'] <= 8000) & (source_df['roi'] >= 0.01) & (source_df['amount'] >= 200)]
#     return result
...
def calculate_roi(groupby_list=['media_id', 'platform', 'image_id']):
    ...
    df = df[(df['media_id'] == 16) & (df['platform'] == 1) & (df['7_amount'] >= 20000) & (df['7_roi'] < 0.06)]  ## TODO
...
def create_plan(df, score_image, image_7):
    ...
    plan['promoted_object_id'] = '2000005658'
...
def main_model():
    ...
    plan_result['bid_amount'] = plan_result['optimization_goal'].apply(
        lambda x: random.randint(11000, 13000) if x == 'OPTIMIZATIONGOAL_APP_ACTIVATE'
    ...
    for i in range(plan_result.shape[0]):
        ...
        a['brand'] = {'brand_name': '迷城起源', 'brand_img': 141595}
