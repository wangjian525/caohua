...
def create_plan(df, score_image):
    ...
    ad_account_id_group = np.array([8814, 8824, 8825, 8826, 9413, 9414, 9415, 9416, 9417, 9420])
...
def main_model():
    ...
    # 调低ROI系数  ##TODO
    plan_result['deep_conversion_worth_spec'] = plan_result['deep_conversion_worth_spec'].apply(lambda x: {'goal': 'GOAL_1DAY_PURCHASE_ROAS', 'expected_roi': 0.01} if x == x else x)
    # 固定出价
    bid_dict = {'OPTIMIZATIONGOAL_APP_ACTIVATE': 17000, 'OPTIMIZATIONGOAL_APP_PURCHASE': 210000,
                'OPTIMIZATIONGOAL_FIRST_PURCHASE': 430000}
    plan_result['bid_amount'] = plan_result['optimization_goal'].map(bid_dict)
