...
def get_score_imag_7():
    ...
    cursor = conn.cursor()
    sql_engine = 'set hive.execution.engine=tez'
    sql = 'select image_id from tmp_data.tmp_ra_media_platform_image where dt= `current_date`() and platform=1 and media_id=10 and score>=60'
    cursor.execute(sql_engine)
    cursor.execute(sql)
    result = as_pandas(cursor)
...
def get_now_plan_roi():
    ...
    # 选择相关指标达标的计划
    result = source_df[(source_df['pay_num']>=2)&(source_df['pay_cost']<=6000)&(source_df['roi']>=0.1)&(source_df['amount']>=200)]
    return result
...
def create_plan(df, score_image, image_7):
    ...
    ad_account_id_group = np.array([6990, 6987])
...
def main_model():
    ...
    plan_result['web_url'] = 'https://www.chengzijianzhan.com/tetris/page/7067351426465234981/'
