...
def get_score_image():
    ...
    result = result[result['label_ids'].isin(
        [19, 25, 81, 103, 112, 136, 152, 20, 36, 41, 78, 81, 98, 119, 126, 27, 60, 93, 102, 103])]
    # 关闭链接
    cursor.close()
    conn.close()
...
def create_plan(df, score_image):
    ...
    # 选budget
    plan['budget'] = plan['platform'].apply(lambda x: 3000 if x == '[ANDROID]' else 4000)
...
def get_train_df():
    # 获取账号、素材、账号+素材维度的数据
    # 读取近期数据
    train_im_1, train_ad_1 = etl_data(start='2021-03-13',
    ...
    # n=3
    train_im_3, train_ad_3 = etl_data(start='2021-03-13',
    ...
    # n=5
    train_im_5, train_ad_5 = etl_data(start='2021-03-13',
    ...
    # n=7
    train_im_7, train_ad_7 = etl_data(start='2021-03-13',
    ...
    #     lambda x: np.inf if x.pay_role_user_num == 0 else x.amount / x.pay_role_user_num,
    #     axis=1)
    ...
    df = df[df['create_time'] >= pd.datetime.now() - pd.DateOffset(30)]
...
def main_model():
    ...
    threshold = pd.Series(y_predict).sort_values(ascending=False).reset_index(drop=True)[int(y_predict.shape[0] * 0.18)]
    ...
    # if plan_result.shape[0] > 10:
    #     plan_result = plan_result.sample(10)
