...
# 获取score_image (分数大于600的image_id)
def get_score_image():
...
def get_now_plan_roi():
    ...
    sql = '''
        ...
        FROM
            ...
            AND a.source_id = b.source_id 
        WHERE
            a.create_time >= date( NOW() - INTERVAL 336 HOUR ) 
            AND b.tdate >= date( NOW() - INTERVAL 336 HOUR )
...
def create_plan(df, score_image):
    ...
    ad_account_id_group = np.array([6621, 6867, 6868])
    ...
    sample_df = sample_df.sample(n=plan.shape[0], replace=True, weights=sample_df['sample_weight']).drop(
        'sample_weight', axis=1).reset_index(drop=True)
...
def get_train_df():
    ...
    # 读取近期数据
    # n=1
    train_im_1, train_ad_1 = etl_data(start='2021-01-31',
    ...
    # n=3
    train_im_3, train_ad_3 = etl_data(start='2021-01-31',
    ...
    # n=5
    train_im_5, train_ad_5 = etl_data(start='2021-01-31',
    ...
    # n=7
    train_im_7, train_ad_7 = etl_data(start='2021-01-31',
    ...
    df = df[df['amount'] >= 500]
    df['create_role_pay_cost'] = df.apply(
        lambda x: np.inf if x.pay_role_user_num == 0 else x.amount / x.pay_role_user_num, axis=1)
    df['platform'] = df['platform'].astype(str)
    df['platform'] = df['platform'].map({"['ANDROID']": 1, "['IOS']": 2})
    df['label'] = df.apply(
        lambda x: 1 if (x.pay_role_user_num > 0) & (x.amount >= 500) & (x.new_role_money / x.amount >= 0.01) else 0,
        axis=1)
    ...
    now_plan_roi['sample_weight'] = 0.2 * now_plan_roi['amount'] + 0.2 * now_plan_roi['roi'] + 0.6 * now_plan_roi[
        'pay_rate']
    ...
    holidays = ['2020-01-01', '2020-01-24', '2020-01-25', '2020-01-26', '2020-01-27', '2020-01-28',
                ...
                '2021-02-12', '2021-02-13', '2021-02-14', '2021-02-15', '2021-02-16', '2021-02-17', '2021-04-03',
                '2021-04-14',
                '2021-04-05', '2021-05-01', '2021-05-02', '2021-05-03', '2021-05-04', '2021-05-05', '2021-06-12',
                '2021-06-13',
                '2021-06-14']
