...
def get_score_image():
    ...
    sql = '''select image_id,label_ids from dws.dws_image_score_d where media_id=10 and score>=560 and dt=CURRENT_DATE and label_ids!='27' group by image_id,label_ids'''
...
def get_score_imag_7():
    ...
    cursor = conn.cursor()
    sql_engine = 'set hive.execution.engine=tez'
    sql = 'select image_id from tmp_data.tmp_ra_media_platform_image where dt= `current_date`() and platform=1 and media_id=10 and score>=60 and mgame_id=1056 and 7_amount>=1000'
    cursor.execute(sql_engine)
    cursor.execute(sql)
    result = as_pandas(cursor)
...
def get_now_plan_roi():
    ...
    # 选择相关指标达标的计划
    result = source_df[(source_df['pay_num'] >= 1) & (source_df['pay_cost'] <= 7000) & (source_df['roi'] >= 0.08) & (source_df['amount'] >= 200)]
    return result
...
def create_plan(ad_account, df_create, df, score_image, image_7):
    ...
    # ad_account_id_group = np.array([10780, 10781, 10782, 10783, 10784, 6990, 6987])
    ...
    # print('df_image', df_create['image_id'].unique())
    ...
    sample_df = df[['manager_id', 'ad_keywords', 'title_list', 'third_industry_id',
                    'inventory_type', 'retargeting_tags_include', 'retargeting_tags_exclude', 'delivery_range', 'city',
         'location_type', 'gender', 'age', 'ac', 'launch_price', 'auto_extend_enabled', 'hide_if_exists', 'hide_if_converted',
         'schedule_time', 'flow_control_mode', 'interest_action_mode', 'action_scene', 'action_days', 'action_categories', 'interest_categories']]
...
def get_train_df():
    ...
    ad_account_id_group = np.array([10782, 10783, 10784, 10780, 10781, 6990])
    plan_create = pd.DataFrame()
    for ad_account in ad_account_id_group:
        df_create_s = df_create[df_create['ad_account_id'] == ad_account]
        print(ad_account, df_create_s.shape[0])
        if df_create_s.shape[0] != 0:
            plan_create_s = create_plan(ad_account, df_create, df_create_s, score_image, image_7)
        else:
            plan_create_s = create_plan(ad_account, df_create, df_create, score_image, image_7)
        plan_create = plan_create.append(plan_create_s)
...
def main_model():
    ...
    ad_account_id_group = np.array([10782, 10783, 10784, 10780, 10781, 6990])
    # plan_num = plan_result['image_id'].nunique()
    # plan_num = 5
    ...
    # 限制机型
    plan_result['launch_price'] = plan_result['launch_price'].apply(lambda x: "[2000, 11000]")
